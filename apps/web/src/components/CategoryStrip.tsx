...
// Each tile owns ONE hue end-to-end (badge gradient + soft card + hover glow),
// so every tile is colourful and none reads as the "odd one out". Full literal
// class strings per item — Tailwind's JIT scans source, so dynamic hues would
// be purged; these stay.
type Item = {
  label: string;
  href: string;
  icon: React.ReactNode;
  card: string; // border + tinted bg + hover glow + focus ring
  badge: string; // gradient icon chip + its shadow
...
const ITEMS: Item[] = [
  {
    ...
    badge: "bg-gradient-to-br from-orange-400 to-orange-600 shadow-orange-500/30",
